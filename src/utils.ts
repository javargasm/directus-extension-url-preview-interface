  ...
  watch(
    ...
    async (val, oldVal) => {
      ...
      for (const key of Object.keys(valObj)) {
        const relation = relations.value.find((rel) =>
          [rel.meta?.one_field, rel.meta?.many_field].includes(key)
        );
        ...
        if (isM2O) {
          if (
            ...
            if ("id" in fieldChanges) {
              fieldChanges = {
                update: [fieldChanges as { id: number | string }],
              };
            } else {
          ...
          if (
            fieldChanges instanceof Array &&
            !(oldValObj[key] instanceof Array)
          ) {
            // When saving, fieldChanges will return to the initial value.
            // We must clear cache to obtain the new value after saving.
            fieldCache = {};
          ...
          if (pkFinal !== "+") {
            ...
            if (key in fieldCache) {
              ...
              data = (
                await api.get(`items/${collection.value}/${pkFinal}`, {
                  params: {
                    fields: [key],
                  },
                })
          ...
          if (fieldChanges.delete) {
            arrayOfIds = arrayOfIds.filter(
              (id) => !fieldChanges.delete!.includes(id)
            );
          }
        }

        if (fieldChanges.update) {
          arrayOfIds = arrayOfIds.concat(
            fieldChanges.update.map(({ id }) => id)
          );
        }
        ...
        if (arrayOfIds.length) {
          ...
          if (relatedCollection) {
            let data;
            if (
              relatedCollection in itemCache &&
              arrayOfIds.every((id) => id in itemCache[relatedCollection])
            ) {
